'''Faça um programa que use a função valorPagamento para determinar o valor a ser pago por uma prestação de uma conta. 
O programa deverá solicitar ao usuário o valor da prestação e o número de dias em atraso e passar estes valores para a função valorPagamento, 
que calculará o valor a ser pago e devolverá este valor ao programa que a chamou. O programa deverá então exibir o valor a ser pago na tela. 
Após a execução o programa deverá voltar a pedir outro valor de prestação e assim continuar até que seja informado um valor igual a zero para a prestação. 
Neste momento o programa deverá ser encerrado, exibindo o relatório do dia, que conterá a quantidade e o valor total de prestações pagas no dia. 
O cálculo do valor a ser pago é feito da seguinte forma. Para pagamentos sem atraso, cobrar o valor da prestação. 
Quando houver atraso, cobrar 3% de multa, mais 0,1% de juros por dia de atraso.'''

# definir a função ValorPagamento()
# Chamar a função e pedir informações do usuário como parâmetro.
# pedir ao usuário informar se tiver dias em atraso, se houver o cálculo será realizado
# O cálculo de juros consiste em 3% do valor de multa + 0.1% de juros por dia de atraso
# exibir o valor na tela e voltar a pedir entradas do usuário até o valor da prestação for 0

def ValorPagamento(prestação,dias_atraso):
    valor = prestação

    if dias_atraso > 0:
        juros = 0.01 * dias_atraso
        valor = (dias_atraso * 0.03) + dias_atraso * juros
        print(valor)

    print(f'\nO valor a ser pago é de: R${valor}')


# Programa principal
print('\nPara encerrar o programa digite o valor como 0 ou menor\n')
while True:
    conta = float(input('Qual valor da conta? ').strip())

    if conta <= 0:
        break

    while True:
        atraso = str(input('\nA conta está atrasada [S/N]? ').strip())
        if atraso in 'Nn':
            break
        atraso = int(input('Quantos dias de atraso? '))
        break

    ValorPagamento(conta, atraso)
